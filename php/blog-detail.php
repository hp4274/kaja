...
if ($slug) {
    $stmt = $db->prepare("SELECT * FROM `blogs` WHERE `slug`=:slug AND `status`='published'");
...
if ($post) {
    $relStmt = $db->prepare("SELECT `id`,`title`,`slug`,`excerpt`,`category`,`read_time`,`cover_image`,`created_at` FROM `blogs` WHERE `status`='published' AND `category`=:cat AND `id`!=:id ORDER BY `created_at` DESC LIMIT 3");
    $relStmt->execute([':cat' => $post['category'], ':id' => $post['id']]);
    $related = $relStmt->fetchAll(PDO::FETCH_ASSOC);
}

// Category colors and emojis for related cards
$catColors = [
    'Anxiety'       => '#f4e8db',
...
// Page meta
$pageTitle = $post ? htmlspecialchars($post['title']) . ' | Rewire With Kajal' : 'Post Not Found | Rewire With Kajal';
$pageDesc  = $post ? htmlspecialchars($post['excerpt']) : 'The blog post you are looking for could not be found.';
?>
<!DOCTYPE html>
<html lang="en">
  <head>
  ...
  <body>
    ...
    <nav class="rw-navbar">
      <div class="rw-container">
        ...
        <button class="navbar-toggler d-lg-none" type="button" style="border:none; background:transparent; font-size:24px;">
    ...
    <?php if ($post): ?>
    <article class="abt2-wrapper blog-detail-page" id="main-content">
      <header class="abt2-story-content blog-detail-block rw-reveal">
        <p class="blog-premium-date-time blog-detail-meta"><?php echo date('F j, Y', strtotime($post['created_at'])); ?> · <?php echo $post['read_time']; ?> min read · <?php echo htmlspecialchars($post['category']); ?></p>
        <h1 class="blog-detail-title"><?php echo htmlspecialchars($post['title']); ?></h1>
        <p class="abt2-hero-desc blog-detail-byline">By <strong>Kajal</strong> · Licensed Therapist</p>
      </header>

      <?php if ($post['cover_image']): ?>
      <div class="abt2-story-content blog-detail-block blog-detail-hero-img rw-reveal">
        <img
      ...
      <div class="abt2-story-content blog-detail-block rw-reveal">
        <div class="abt2-story-text">
          <?php echo $post['content']; ?>
        </div>
      </div>

      <?php if (!empty($related)): ?>
      <div class="abt2-story-content blog-detail-block rw-reveal" style="margin-top:2rem;">
        <h2 style="font-size:1.35rem; font-weight:600; margin-bottom:1.25rem;">Related Articles</h2>
        <div class="blog-premium-grid" style="display:grid; grid-template-columns:repeat(auto-fill, minmax(260px, 1fr)); gap:1.25rem;">
          <?php foreach ($related as $rel): ?>
            <article class="blog-premium-card">
              <a href="blog-detail.php?slug=<?php echo urlencode($rel['slug']); ?>" class="text-decoration-none text-reset">
                <div class="blog-premium-card-img" style="background: <?php echo $catColors[$rel['category']] ?? '#f4e8db'; ?>">
                  <?php if ($rel['cover_image']): ?>
                    <img src="<?php echo htmlspecialchars($rel['cover_image']); ?>" alt="" loading="lazy" style="width:100%;height:100%;object-fit:cover;" />
                  <?php else: ?>
                    <div class="blog-premium-emoji-fallback" style="font-size: 3rem"><?php echo $catEmojis[$rel['category']] ?? '📝'; ?></div>
                  <?php endif; ?>
                  <div class="blog-premium-card-cat"><?php echo htmlspecialchars($rel['category']); ?></div>
                </div>
                <div class="blog-premium-card-body">
                  <div class="blog-premium-card-meta"><?php echo date('F j, Y', strtotime($rel['created_at'])); ?> · <?php echo $rel['read_time']; ?> min read</div>
                  <h3 class="blog-premium-card-title"><?php echo htmlspecialchars($rel['title']); ?></h3>
                  <p class="blog-premium-card-excerpt"><?php echo htmlspecialchars($rel['excerpt']); ?></p>
